...
void analyse_line(char *line, evt_t *evt)
{
...
void read_evt(FILE *file, char *line, evt_list_t **list)
{
    ...
    do {
        ...
        if (nread == -1)
            break;
        line[nread-1] = '\0';
        analyse_line(line, &event);
    } while (nread != -1 && my_strcmp(line, "-evt") != 1);
    if (event.type != 0)
        put_event_in_list(list, event);
    if (my_strcmp(line, "-evt") == 1)
        read_evt(file, line, list);
}
...
void read_evts(char *path, evt_list_t **list)
{
    ...
    do {
        ...
        if (nread == -1)
            ...
            line[nread-1] = '\0';
        if (nread > 0 && my_strcmp(line, "-evt") == 1)
            read_evt(file, line, list);
    } while (nread != -1);
    if (line != NULL)
        free(line);
