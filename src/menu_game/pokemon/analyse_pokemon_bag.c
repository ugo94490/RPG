...
static void analyse_keyboard_pkmn_bag_game(pkmn_bag_t *pkmn_bag)
{
    ...
    if (sfKeyboard_isKeyPressed(sfKeyRight) && pkmn_bag->n_slot + 1 <= pkmn_bag->n_pkmn)
        pkmn_bag->n_slot += 1;
    if (sfKeyboard_isKeyPressed(sfKeyLeft) && pkmn_bag->n_slot - 1 >= 1)
        pkmn_bag->n_slot -= 1;
    if (sfKeyboard_isKeyPressed(sfKeyDown) && pkmn_bag->n_slot + 2 <= pkmn_bag->n_pkmn)
        pkmn_bag->n_slot += 2;
...
{
    sfVector2i pMouse = sfMouse_getPositionRenderWindow(pkmn_bag->window->window);

    for (int i = 0; i < pkmn_bag->n_pkmn; i += 1)
        if ((float)pMouse.x >= pkmn_bag->slot[i].pos.x &&
        (float)pMouse.x <= pkmn_bag->slot[i].pos.x + pkmn_bag->slot[i].rect.width
        && (float)pMouse.y >= pkmn_bag->slot[i].pos.y && (float)pMouse.y
...
void analyse_pokemon_bag(pkmn_bag_t *pkmn_bag)
{
    ...
    while (sfRenderWindow_pollEvent(pkmn_bag->window->window, &event)) {
        ...
        //     analyse_click_pkmn_bag_game(pkmn_bag);
